...
module.exports = function (app, con, db) {
  ...
  app.get('/getRezultatiAnkete', function (req, res) {
    test(req.query.username, req.header('Authorization'), req, res, (req, res) => {
      let idKorisnik = req.query.idKorisnik || 0
      let idAnk = req.query.idAnketa || 0
      con.query(`SELECT * 
                FROM Anketa a, Korisnik k
                WHERE (a.tipAnkete = 'javna anketa'
                OR a.idNapravio = ${idKorisnik} 
                OR k.idUloga = 4)
                AND a.idAnketa = ${idAnk}
                `, (error, ank) => {
        if(error || ank.length == 0) {
          res.json({accessError: "Ne možete pristupiti rezultatima ove ankete."})
          return
        }
